...
if [[ ! -f $SERVER_SCRIPT ]]; then
    ...
    if [[ ! -z $(which wget) ]]; then
        wget --tries=3 --timeout=10 --continue -O vscode-server.tar.gz $SERVER_DOWNLOAD_URL
    elif [[ ! -z $(which curl) ]]; then
        curl --retry 3 --connect-timeout 10 --location --output vscode-server.tar.gz $SERVER_DOWNLOAD_URL
    else
